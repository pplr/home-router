...
| `wifi/psk-trusted` | `apbuild.render` (via the `trusted` `[[aps.ssids]]` entry's `psk_secret` in `config.toml`) | PSK for `maison-jaune` (SAE/WPA3) |
| `wifi/psk-iot` | `apbuild.render` (via the `iot` `[[aps.ssids]]` entry's `psk_secret` in `config.toml`) | PSK for `maison-jaune-iot` (WPA2-PSK) |
...
per AP whose `[aps.<label>]` table sets `cert = true`. `<label>` is that
table key — the AP's single identifier, which is also the leftmost DNS
label of its certificate (e.g. `ax59u` for `ax59u.ap.verson.lplr.eu`).

The set of PSK files is **data-driven**: it follows from the `psk_secret` paths declared in `config.toml`'s `[[aps.ssids]]` entries. Add an SSID there, and a new PSK file becomes required here.
...
#    Generates one directory per AP with `cert = true` in config.toml.
