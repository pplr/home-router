...
**`core-image-minimal` ships *no* locale, and nothing sets `LANG` — both halves must be fixed:** OE-core's `core-image-minimal.bb` hard-assigns `IMAGE_LINGUAS = " "`, overriding the `c en-us en-gb` default from `default-distrovars.inc`, so `image.bbclass` computes an empty `LINGUAS_INSTALL` and not a single `locale-base-*` package reaches the rootfs — `/usr/lib/locale` is empty and `setlocale()` can only ever land on glibc's built-in ASCII `C`. Separately, OE's `/etc/profile` exports only `PATH PS1 … EDITOR TERM` (never `LANG`), there is no `/etc/locale.conf`, and OE's `sshd_config` carries no `AcceptEnv`, so the client's locale isn't forwarded either. Symptom: `btop` exits with `ERROR: No UTF-8 locale detected!` (it requires `setlocale(LC_ALL, "")` to return a name ending in `UTF8`, or `LANG`/`LC_ALL`/`LC_CTYPE` to hold one); more generally anything multibyte-aware sees an ASCII codeset. Fix, all four pieces:

- `IMAGE_LINGUAS = "c"` in `core-image-minimal.bbappend` — **not** `local.conf`, since conf files are parsed *before* recipes and a recipe's plain `=` always wins.
- `/etc/locale.conf` (`LANG=C.UTF-8`) — read by systemd PID1 and folded into every unit's default environment.
- `/etc/profile.d/locale.sh` — sshd builds a fresh environment for user sessions (`do_setup_env()` forwards `TZ` and nothing else), so a login shell needs its own export.
- `SetEnv LANG=C.UTF-8` in the `sshd_config.d` drop-in — a *non*-login shell (`ssh -t home-router btop`) sources neither of the above.

All three files are shipped by `futro-console-conf` / the openssh bbappend. Unlike the openssh trap above, no `FILES` reclaim is needed for `futro-console-conf`: the default `FILES:${PN}` already covers `${sysconfdir}`. Do **not** use `/etc/environment` for this — `pam-plugin-env` already owns that path and the duplicate would fail packaging.

**`locale-base-c` *is* `C.UTF-8`, and the on-disk name is plain `C`:** `LOCALE_UTF8_IS_DEFAULT = "1"` (oe-core default) makes `libc-package.bbclass` strip the `.UTF-8` suffix from every UTF-8 locale, so glibc's `C.UTF-8` is packaged as `locale-base-c` and installed as `/usr/lib/locale/C` — 375 KB, versus 2.96 MB for `en_US`/`fr_FR` (the difference is the full `LC_COLLATE` table). It really is UTF-8: its `LC_CTYPE` is 369120 bytes, byte-for-byte the size of `en_US`'s, against 315184 for `en_US.ISO-8859-1`. `LANG=C.UTF-8` still resolves to it for two reasons worth knowing before changing the locale: glibc's `C`/`POSIX` short-circuit in `_nl_find_locale` fires only on the *exact* string `C`, so `"C.UTF-8"` goes through the normal file lookup (whose `_nl_make_l10nflist` candidate list includes bare `C`); and `IMAGE_LOCALES_ARCHIVE ?= '1'` makes the rootfs postprocess fold the directory into `/usr/lib/locale/locale-archive` under its *normalized* name — the codeset from `LC_CTYPE` is appended, giving `C.utf8`, exactly what `C.UTF-8` normalizes to. Note `locale(1)` is not in the image (`glibc-utils` isn't installed), so verify with `echo $LANG` + `ls /usr/lib/locale/` rather than `locale -a`. `GLIBC_GENERATE_LOCALES` in `conf/distro/home-router.conf` pins generation to that one locale; left unset, glibc builds every entry in its `SUPPORTED` file (~1900) for the image to discard.
